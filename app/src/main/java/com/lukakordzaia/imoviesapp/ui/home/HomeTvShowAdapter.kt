...
class HomeTvShowAdapter(private val context: Context, private val onTvShowClick: (id: Int) -> Unit) : RecyclerView.Adapter<HomeTvShowAdapter.ViewHolder>() {
    ...
    fun setTvShowsList(list: List<MoviesList.Data>) {
        this.list = list
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(
                LayoutInflater.from(context).inflate(R.layout.rv_tvshow_item, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.movieRoot.setOnClickListener {
            listModel.id?.let { movieId -> onTvShowClick(movieId) }
        }
